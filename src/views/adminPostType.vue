          ...
          <div>
            ...
            >
              ...
              <div class="article-text">
                <div class="article-title">{{ item.type_name }}</div>
...
export default {
  ...
  methods: {
    ...
    addTypeClick() {
      if (!this.titleInput) {
        _methods.tanChuang(this, "请输入帖子类型名称");
        return;
      }
      if (!this.describeInput) {
        _methods.tanChuang(this, "请输入帖子类型描述");
        return;
      }
      let publicData = {
        type_name: this.titleInput,
        describe: this.describeInput,
        yg_id: this.userInfo.yg_id
      };
      let jiami = {
        type_name: this.titleInput,
      ...
      Services.loginApi.addType(publicData, jiami).then(res => {
        if (res.event == 100) {
    ...
    deletePost(item) {
      ...
      Services.loginApi.delType(publicData, jiami).then(res => {
        if (res.event == 100) {
          // _methods.tanChuangOk(this, res.msg);
          console.log("我的内容", res);
          this.getTypeList();
